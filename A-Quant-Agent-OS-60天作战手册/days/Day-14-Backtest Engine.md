...
- Git Commit：`feat: add backtest engine`
...
```text
你正在开发 A-Quant Agent OS 的 Day 14：Backtest Engine。
...
实现 Backtest Engine，接收 Strategy 和历史数据，输出 trades、equity curve、metrics。
...
## Code Review Prompt
```text
请作为 A-Quant Agent OS 的 Staff Engineer Review Day 14：Backtest Engine。
...
请对 Day 14：Backtest Engine 做严格验收。
