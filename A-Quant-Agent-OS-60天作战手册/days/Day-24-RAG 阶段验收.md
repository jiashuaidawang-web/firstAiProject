...
- Git Commit：`test: validate financial rag`
...
```text
你正在开发 A-Quant Agent OS 的 Day 24：RAG 阶段验收。
...
执行固定 RAG 问题集，输出 retrieval、answer、citation 和 eval report。
...
## Code Review Prompt
```text
请作为 A-Quant Agent OS 的 Staff Engineer Review Day 24：RAG 阶段验收。
