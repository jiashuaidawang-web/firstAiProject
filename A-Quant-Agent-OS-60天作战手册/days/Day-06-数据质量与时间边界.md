...
## 今日目标
建立缺失、重复、时间倒序、异常价格等数据质量检查。

## 最终产物
- 根据今日需求生成代码、测试、文档和可运行结果
- Git Commit：`feat: add market data quality checks`
...
```text
你正在开发 A-Quant Agent OS 的 Day 06：数据质量与时间边界。
...
实现 MarketDataQualityValidator，检查缺失、重复、时间顺序、异常价格和未来时间数据。
...
## Code Review Prompt
```text
请作为 A-Quant Agent OS 的 Staff Engineer Review Day 06：数据质量与时间边界。
...
## Git Commit
```text
feat: add market data quality checks
```
