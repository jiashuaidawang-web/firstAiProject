...
- Git Commit：`feat: add backtest performance metrics`
...
```text
你正在开发 A-Quant Agent OS 的 Day 16：回测指标。
...
实现回测指标计算，并对边界情况（无交易、常量收益、空序列）增加测试。
...
## Code Review Prompt
```text
请作为 A-Quant Agent OS 的 Staff Engineer Review Day 16：回测指标。
...
## Git Commit
```text
feat: add backtest performance metrics
```
