...
- Git Commit：`test: validate market data foundation`
...
```text
你正在开发 A-Quant Agent OS 的 Day 10：数据底座阶段验收。
...
对数据层做端到端验收：Provider→Normalize→Quality→ClickHouse→API。
...
## Code Review Prompt
```text
请作为 A-Quant Agent OS 的 Staff Engineer Review Day 10：数据底座阶段验收。
...
特别检查：
严格列出 P0/P1/P2，不允许未解决 P0 进入量化阶段。
...
## Git Commit
```text
test: validate market data foundation
```
