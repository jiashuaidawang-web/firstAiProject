...
- Git Commit：`feat: add supervisor routing`
...
```text
你正在开发 A-Quant Agent OS 的 Day 40：Supervisor / Router。
...
实现 Supervisor，根据任务选择单 Agent、多个 Agent 或拒绝不支持任务。
...
## Code Review Prompt
```text
请作为 A-Quant Agent OS 的 Staff Engineer Review Day 40：Supervisor / Router。
...
请对 Day 40：Supervisor / Router 做严格验收。
