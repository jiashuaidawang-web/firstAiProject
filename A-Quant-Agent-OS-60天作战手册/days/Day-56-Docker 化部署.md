...
- Git Commit：`feat: dockerize a-quant platform`
...
## 🔴 必须理解
容器化不是把 jar 放进 Docker；依赖、配置、健康检查都要考虑。
...
```text
你正在开发 A-Quant Agent OS 的 Day 56：Docker 化部署。
...
为核心服务创建 Docker Compose，包含依赖服务和 healthcheck。
...
## Code Review Prompt
```text
请作为 A-Quant Agent OS 的 Staff Engineer Review Day 56：Docker 化部署。
...
## Git Commit
```text
feat: dockerize a-quant platform
```
